...
export const startInterview = async (req, res, next) => {
  try {
    ...
    // Truncate resume text to prevent context limit issues (approx 15k words)
    const truncatedResume = resumeText.substring(0, 50000);
    ...
    console.error("CRITICAL Interview Start Error:", error);
...
export const submitTextAnswer = async (req, res, next) => {
  try {
    ...
    if (!answer || answer.trim() === "") {
      return res
        .status(400)
        .json({ success: false, message: "Please provide an answer." });
    }
...
export const speakText = async (req, res, next) => {
  try {
    ...
    if (!text) {
      return res
        .status(400)
        .json({ success: false, message: "No text provided for speech." });
    }
